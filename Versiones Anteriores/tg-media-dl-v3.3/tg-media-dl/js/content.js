...
(function () {
  ...
  async function downloadItem(item, onProgress) {
    ...
    if (type === "gif" || type === "video") {
      ...
      // If we have a src (stream URL) — download via injected.js
      ...
      // Unloaded video with stale element — we can't download without loading it
      if (unloaded && !src) {
        log.w(`Video no cargado, no se puede descargar desde galería: ${name}. Ábrelo en el visor para descargarlo.`);
        return false;
      }
      if (src) return downloadUrl(src, name);
      return false;
    ...
    if (type === "doc") {
      ...
      log.w(`Doc: ${name} — abre el chat y descárgalo desde ahí`);
      return false;
    }
    return false;
  }
  ...
  // Main scanner: Opens Shared Media sidebar and scrolls each tab
  async function sharedMediaScan(onMedia, onProgress, signal) {
    const allMedia = [];
    const seenMids = new Set();

    // Step 1: Open the profile/shared media panel
    log.i("Abriendo panel de Shared Media...");
    if (onProgress) onProgress("scanning", 0, 0);

    // Click the chat header to open the profile sidebar
    const chatHeader = document.querySelector(".chat-info, .sidebar-header .person, .top .chat-info-container, .chat-utils .btn-icon.sidebar-close-button");
    // Try different methods to open the sidebar
    let sidebarOpened = false;

    // Method 1: Click the chat title/avatar area
    const chatTitle = document.querySelector(".chat-info .content, .chat-info .person, .top .person");
    if (chatTitle) {
      chatTitle.click();
      await sleep(500);
    }

    // Method 2: If that didn't work, try the peer title
    if (!document.querySelector(".profile-container, .shared-media-container, .search-super")) {
      const peerTitle = document.querySelector(".chat-info .peer-title, .top .peer-title");
      if (peerTitle) {
        peerTitle.click();
        await sleep(500);
      }
    ...
    // Method 3: Try the avatar
    if (!document.querySelector(".profile-container, .shared-media-container, .search-super")) {
      const avatar = document.querySelector(".chat-info .avatar-element, .chat-info .peer-photo");
      if (avatar) {
        avatar.click();
        await sleep(500);
      }
    ...
    // Wait for the profile sidebar to appear
    let profile = null;
    for (let i = 0; i < 20; i++) {
      if (signal?.aborted) return allMedia;
      profile = document.querySelector(".profile-container, #column-right .sidebar-slider");
      if (profile) { sidebarOpened = true; break; }
      await sleep(300);
    }

    if (!sidebarOpened) {
      log.w("No se pudo abrir el sidebar. Intentando scan de chat directo...");
      return chatScrollScan(onMedia, onProgress, signal);
    }

    log.i("Sidebar abierto, buscando tabs de media...");
    await sleep(800); // Wait for tabs to render

    // The media tabs in Telegram K are in .search-super .search-super-tabs
    // Tab types: members, media (photos), files, links, music, voice
    // We scan: media (photos+videos+gifs), files, music
    const tabConfigs = [
      { name: "media", selector: '.search-super-tab[data-type="members"]', alt: null },
    ];

    // Find the search-super container (shared media section)
    let searchSuper = document.querySelector(".search-super");

    // If no search-super yet, we might need to scroll down in the profile to reveal it
    // or click a "Shared Media" link
    if (!searchSuper) {
      // Look for a "shared media" or "media" link in the profile
      const profileContent = document.querySelector(".profile-content .scrollable, .profile-container .scrollable");
      if (profileContent) {
        // Scroll down to find shared media section
    ...
    if (!searchSuper) {
      log.w("No se encontró la sección de Shared Media. Fallback a chat scroll...");
      // Close sidebar
      const closeBtn = document.querySelector(".sidebar-close-button, #column-right .btn-icon.sidebar-close-button");
      if (closeBtn) closeBtn.click();
      return chatScrollScan(onMedia, onProgress, signal);
    }

    // Now scan the media grid tabs
    // Each tab shows items in .search-super-content-* containers
    // Photos/videos appear in a grid of .search-super-item elements
    const tabs = searchSuper.querySelectorAll(".search-super-tab");
    log.i(`Encontrados ${tabs.length} tabs en Shared Media`);

    // Helper: scan all items currently visible in the search-super content
    function scanSharedMediaItems() {
      let newCount = 0;
      // Photos and videos in the media grid
      searchSuper.querySelectorAll(".search-super-item").forEach((item) => {
        ...
        const videoTime = item.querySelector(".video-time, span.video-time");
        const videoIcon = item.querySelector(".video-play");
        if (videoTime || videoIcon) {
          type = "video";
        ...
        // Check for GIF
        const gifBadge = item.querySelector(".video-time.gif");
        if (gifBadge || item.querySelector(".gif-badge")) {
          type = "gif";
          name = `gif_${mid}.mp4`;
        }
        ...
        if (canvas && !thumb) {
          try { thumb = canvas.toDataURL("image/jpeg", 0.6); } catch (_) {}
        }

        // For photos, try to get a stream URL from data attributes
        if (type === "photo" && !name) {
          name = `photo_${mid}.jpg`;
        }
        ...
        const serialized = {
          ...
          name,
          isCanvas: false,
          unloaded: type !== "photo", // videos/gifs need to be opened to download
          el: item,
          _msgId: mid,
          _fromSharedMedia: true, // Flag so download knows to use media viewer
        };
      ...
      searchSuper.querySelectorAll(".document-with-thumb, .document-container, .search-super-content-files .row").forEach((item) => {
        const mid = item.dataset?.mid || item.closest("[data-mid]")?.dataset?.mid;
        if (!mid || seenMids.has(mid)) return;
        seenMids.add(mid);

        const nameEl = item.querySelector(".document-name, .text-bold");
    ...
    // Click each tab and scroll its content
    const mediaTypes = ["media", "files", "music"];
    for (const tabType of mediaTypes) {
      ...
      tabs.forEach((t) => {
        ...
        const dataType = t.dataset?.type || "";
        if (tabType === "media" && (label.includes("media") || label.includes("foto") || label.includes("photo") || dataType === "members" || dataType === "media")) {
          tab = t;
        }
        if (tabType === "files" && (label.includes("file") || label.includes("archivo") || dataType === "files")) {
          tab = t;
        }
        if (tabType === "music" && (label.includes("music") || label.includes("música") || label.includes("voice") || label.includes("voz") || dataType === "music" || dataType === "voice")) {
          tab = t;
        }
      });

      if (!tab) {
        // Try by index: usually tab[0]=members, tab[1]=media, tab[2]=files, tab[3]=links, tab[4]=music
        if (tabType === "media" && tabs[1]) tab = tabs[1];
        if (tabType === "files" && tabs[2]) tab = tabs[2];
        if (tabType === "music" && tabs[4]) tab = tabs[4];
      }
      ...
      log.i(`Escaneando tab: ${tabType}...`);

      // Find the scrollable content area for this tab
      const content = searchSuper.querySelector(".search-super-content.active, .tabs-tab.active .scrollable, .search-super-content:not(.hide)");
      let scrollContainer = content?.querySelector(".scrollable") || content?.closest(".scrollable") || searchSuper.querySelector(".scrollable");

      if (!scrollContainer) {
        // Try the profile's main scrollable
        scrollContainer = document.querySelector("#column-right .scrollable, .profile-content .scrollable");
      }

      if (!scrollContainer) {
        log.w(`No scrollable para tab ${tabType}`);
        scanSharedMediaItems();
        continue;
      }
      ...
      while (scrollAttempts < 200 && staleRounds < MAX_STALE) {
        if (signal?.aborted) break;

        const prevCount = allMedia.length;

        // Scroll down in the shared media grid
        ...
        for (let poll = 0; poll < 5; poll++) {
          await sleep(300);
          const newItems = scanSharedMediaItems();
          if (newItems > 0) break;
        }
        ...
        if (scrollAttempts % 10 === 0) {
          log.i(`Shared Media [${tabType}]: ${allMedia.length} items, ${scrollAttempts} scrolls, stale=${staleRounds}`);
        }
      }
    }

    // Close the sidebar to return to normal view
    await sleep(300);
    const closeBtn = document.querySelector(".sidebar-close-button, #column-right .btn-icon.sidebar-close-button");
    if (closeBtn) closeBtn.click();

    if (onProgress) onProgress("done", allMedia.length, 0);
    log.i(`Shared Media scan completo: ${allMedia.length} items (${seenMids.size} mids únicos)`);
    return allMedia;
  }
      ...
      try {
        switch (msg.action) {
          ...
          case "autoScrollAndGallery": {
            ...
            // Add a stop button to the toast
            if (toastEl) {
