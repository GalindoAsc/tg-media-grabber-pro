...
(function () {
  ...
  /**
   * Listen for messages from injected.js about captured media
   */
  window.addEventListener("message", (event) => {
    if (event.source !== window) return;

    if (event.data?.type === "TG_GRABBER_MEDIA_READY") {
      const { url, blobUrl, mime, size, fileName } = event.data;
      log.i(`Interceptado: ${fileName || url} (${humanSize(size)}) [${mime}]`);

      // Store for later use
      S.capturedMedia.set(url, { blobUrl, mime, size, fileName });

      // If there's a pending download waiting for this URL, resolve it
      for (const [key, pending] of S.pendingDownloads.entries()) {
        // Match by partial URL
        if (url.includes(key) || key.includes(url)) {
          pending.resolve({ blobUrl, mime, size, fileName: fileName || pending.fileName });
          S.pendingDownloads.delete(key);
          break;
        }
      }
    }

    if (event.data?.type === "TG_GRABBER_MEDIA_CHUNK") {
      const { url, received, total, fileName } = event.data;
      if (total) {
        const pct = Math.round((received / total) * 100);
        if (pct % 20 === 0) { // Log every 20%
          log.i(`Descargando ${fileName || "video"}: ${pct}% (${humanSize(received)}/${humanSize(total)})`);
        }
      }
    }
  });
  ...
  /**
   * Inject the network interceptor into page context
   */
   ...
   * 1. Check if we already captured this video's data
   * 2. If not, trigger the video to load/play so Telegram fetches it
   * 3. Wait for the interceptor to capture the full stream
   * 4. Download the captured blob
   */
  async function downloadVideoSmart(videoEl, fileName, onProgress) {
    ...
    log.i(`Video download: src=${src.substring(0, 80)}...`);

    // Strategy 1: Check if already captured
    for (const [url, data] of S.capturedMedia.entries()) {
      if (data.fileName === fileName || (src && url.includes(src.replace("blob:", "")))) {
        log.i(`Usando media ya capturado: ${data.fileName || fileName}`);
        const finalName = data.fileName || fileName;
        await downloadUrl(data.blobUrl, finalName);
        return true;
      }
    }

    // Strategy 2: For non-blob URLs, try direct chunked download
    if (src && !src.startsWith("blob:")) {
      try {
        return await downloadVideoChunked(src, fileName, onProgress);
      } catch (e) {
        log.w(`Direct download failed: ${e.message}`);
      }
    }

    // Strategy 3: Trigger video load and wait for interceptor to capture
    log.i("Activando video para captura por interceptor...");
    return new Promise((resolve) => {
      // Register a pending download listener
      const timeoutId = setTimeout(() => {
        // Timeout - try fallback approaches
        log.w("Timeout esperando interceptor, intentando fallback...");
        fallbackVideoDownload(videoEl, fileName).then(resolve);
      }, 15000);

      // Listen for the interceptor to capture the media
      const handler = (event) => {
        if (event.data?.type === "TG_GRABBER_MEDIA_READY") {
          const { blobUrl, mime, size, fileName: capturedName } = event.data;
          // Check if this is likely our video (by timing)
          clearTimeout(timeoutId);
          window.removeEventListener("message", handler);
          const finalName = capturedName || fileName;
          const ext = extFromMime(mime);
          const fixedName = finalName.match(/\.(mp4|webm|mkv|mov|avi|ogg|mp3)$/i)
            ? finalName : finalName.replace(/\.[^.]+$/, `.${ext}`);
          log.i(`Capturado por interceptor: ${fixedName} (${humanSize(size)})`);
          downloadUrl(blobUrl, fixedName).then(() => resolve(true));
        }
      };
      window.addEventListener("message", handler);

      // Trigger the video to play/load so Telegram fetches the full file
      try {
        // Click the video or its play button to trigger loading
        const playBtn = videoEl.closest(".VideoPlayer, [class*='VideoPlayer'], .media-viewer-aspecter")
          ?.querySelector('button, [class*="play"], .btn-circle');

        if (videoEl.paused) {
          videoEl.currentTime = 0;
          videoEl.play().catch(() => {
            // If autoplay blocked, try clicking play button
            if (playBtn) playBtn.click();
          });
        } else {
          // Already playing, seek to start to trigger full fetch
          videoEl.currentTime = 0;
        }
      } catch (e) {
        log.w("Error triggering video: " + e.message);
      }
    });
  }

  /**
   * Fallback: use MediaRecorder or direct blob fetch
   */
  async function fallbackVideoDownload(videoEl, fileName) {
    const src = videoEl.src || videoEl.currentSrc;

    // Try 1: Direct blob fetch (might work for small videos)
    if (src) {
      try {
        const resp = await fetch(src);
        const blob = await resp.blob();
        if (blob.size > 50000) { // > 50KB means probably something useful
          log.i(`Blob fetch: ${humanSize(blob.size)}`);
          const ext = extFromMime(blob.type);
          const fixedName = fileName.replace(/\.[^.]+$/, `.${ext}`);
    ...
    try {
      ...
      return new Promise((resolve) => {
        ...
        recorder.onstop = async () => {
          ...
          const fixedName = fileName.replace(/\.[^.]+$/, `.${ext}`);
          log.i(`MediaRecorder: ${humanSize(blob.size)}`);
          await downloadBlob(blob, fixedName);
          resolve(true);
        };
        recorder.onerror = () => { log.e("MediaRecorder error"); resolve(false); };
      ...
      log.e(`MediaRecorder fallback failed: ${e.message}`);
    }

    // Try 3: Just download whatever we have
    if (src) {
  ...
  async function downloadItem(item, onProgress) {
    ...
    if (type === "photo" && isCanvas) {
      return new Promise((res) => {
        el.toBlob((b) => { if (b) downloadBlob(b, name.replace(/\.[^.]+$/, ".png")); res(); }, "image/png");
      });
    }
    if (type === "photo") {
    ...
    if (type === "audio") {
      if (src && !src.startsWith("blob:")) return downloadUrl(src, name);
      // Audio blob - similar problem as video
      if (src) {
        try {
