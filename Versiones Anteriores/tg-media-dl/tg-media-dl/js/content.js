...
(function () {
  ...
  function detectVersion() {
    if (document.querySelector(".bubbles-inner, #column-center .bubbles")) return "K";
    if (document.querySelector("#MiddleColumn .messages-container, .MessageList")) return "A";
    // Fallback by URL
    if (S.isK) return "K";
    if (S.isA) return "A";
    return "K"; // default
  ...
  const SEL = {
    ...
    A: {
      chatContainer: ".MessageList, .messages-container, #MiddleColumn",
      message: ".Message",
      mediaViewer: "#MediaViewer",
      mediaViewerMedia: ".MediaViewerSlide--active .MediaViewerContent",
      mediaViewerButtons: ".MediaViewerActions",
      mediaViewerVideo: ".VideoPlayer video",
      mediaViewerImg: "div > img",
      stories: "#StoryViewer",
      storyVideo: "video",
      storyImg: "img",
    },
  ...
  function findMediaInMessage(msg, version) {
    ...
    const imgSels = v === "K"
      ...
      : ".Photo img, .media-inner img, img.full-media, img.thumbnail";
    ...
    const videoSels = v === "K"
      ...
      : ".VideoPlayer video, video.full-media";
  ...
  async function downloadItem(item, onProgress) {
    ...
    if (type === "photo" || type === "gif" || type === "audio") {
      if (src) return downloadUrl(src, name);
    }
    if (type === "video") {
      if (!src) return false;
      try {
        return await downloadVideoChunked(src, name, onProgress);
      } catch (_) {
    ...
    if (type === "doc") {
      // Documents usually need their in-page download button clicked
      ...
      dlBtn.addEventListener("click", async (e) => {
        ...
        try {
          if (video) {
            ...
            const name = extractFileName(src, video, "video");
            await downloadVideoChunked(src, name, null);
          } else if (img?.src) {
  ...
  function watchStories() {
    ...
    const observer = new MutationObserver(() => {
      ...
      dlBtn.addEventListener("click", async (e) => {
        ...
        if (vSrc) {
          await downloadVideoChunked(vSrc, extractFileName(vSrc, video, "video"), null).catch(() => downloadUrl(vSrc, extractFileName(vSrc, video, "video")));
        } else {
    ...
    switch (msg.action) {
      ...
      case "autoScrollScan": {
        ...
        autoScrollScan(null, (status, count) => {
          chrome.runtime.sendMessage({ action: "scanProgress", status, count });
        }, ac.signal).then((media) => {
          S.scannedMedia = media;
          chrome.runtime.sendMessage({ action: "scanComplete", count: media.length });
        });
        sendResponse({ started: true });
        break;
      }
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg.action === "command" && msg.command === "download-current") {
      // Try to download the media currently in viewer or hovered
      const v = detectVersion();
      const viewer = document.querySelector(SEL[v].mediaViewer);
      if (viewer) {
        const btn = viewer.querySelector(".tg-grab-viewer-btn");
        if (btn) btn.click();
      }
    }
    if (msg.action === "command" && msg.command === "open-gallery") {
      const media = S.scannedMedia.length ? S.scannedMedia : scanVisible().media;
      openGallery(media);
    }
  });
