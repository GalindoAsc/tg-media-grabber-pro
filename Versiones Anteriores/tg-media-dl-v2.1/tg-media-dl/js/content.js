...
(function () {
  ...
  /**
   * For video elements in Telegram Web, the src is often a blob: URL
   * that only contains the buffered portion. We need to find the real
   * stream URL from the video element or use MediaRecorder as fallback.
   */
  function getRealVideoSrc(videoEl) {
    // 1. Check if there's a non-blob src directly
    ...
    // 3. Look for stream URL in parent elements' data attributes
    const container = videoEl.closest(
      ".VideoPlayer, [class*='VideoPlayer'], .media-viewer-aspecter, .MediaViewerContent, [class*='MediaViewerContent']"
    );
    if (container) {
      // Telegram sometimes stores the URL in data attributes or href
      const links = container.querySelectorAll("a[href]");
      for (const link of links) {
        if (link.href && !link.href.startsWith("blob:") && /stream|video|mp4/i.test(link.href)) {
          return link.href;
        }
      }
    }

    // 4. Try to parse the blob URL path — Telegram Web A uses format:
    //    stream/{"dcId":N,"location":{...},"size":N,"mimeType":"video/mp4","fileName":"xxx.mp4"}
    //    The actual fetch goes to a service worker which is why it's blob:
    //    We can construct the stream URL from page context
    
    // 5. Return blob URL as last resort (we'll handle it specially)
    return directSrc;
  }

  async function downloadVideoSmart(videoEl, fileName, onProgress) {
    const src = getRealVideoSrc(videoEl);
    ...
    // If it's a real URL (not blob), use chunked download
    if (!src.startsWith("blob:")) {
      try {
        return await downloadVideoChunked(src, fileName, onProgress);
      } catch (e) {
        log.w(`Chunked failed: ${e.message}, trying direct...`);
        return await downloadUrl(src, fileName);
      }
    }

    // For blob: URLs — try to capture the full video using MediaRecorder
    // But first, try a simple fetch in case the blob contains the full file
    try {
      const resp = await fetch(src);
      const blob = await resp.blob();
      
      // Check if the blob is reasonably sized (>100KB suggests it's complete)
      if (blob.size > 100000) {
        log.i(`Video blob: ${(blob.size / 1048576).toFixed(1)} MB — descargando`);
        const ext = extFromMime(blob.type);
        if (!fileName.match(/\.(mp4|webm|mkv|mov|avi)$/i)) {
          fileName = fileName.replace(/\.[^.]+$/, `.${ext}`);
        }
      ...
      log.w(`Blob fetch partial: ${e.message}`);
    }

    // Use MediaRecorder to capture the full video by playing it
    log.i("Usando MediaRecorder para capturar video completo...");
    return new Promise((resolve) => {
      try {
        ...
        recorder.onstop = async () => {
          ...
          log.i(`MediaRecorder: ${(finalBlob.size / 1048576).toFixed(1)} MB capturados`);
        ...
        // Play video from start and record
        ...
        // Safety timeout (max 10 min recording)
        setTimeout(() => {
        ...
        log.e(`MediaRecorder not available: ${e.message}`);
        // Final fallback: just download whatever the blob has
        downloadUrl(src, fileName).then(resolve);
      }
    });
  }
  ...
  async function downloadItem(item, onProgress) {
    ...
    if (type === "gif") {
      // GIFs in Telegram are actually short videos
      if (el?.tagName === "VIDEO") {
    ...
    if (type === "doc") {
      ...
      log.w(`Doc sin botón de descarga: ${name}`);
      return false;
    }
    return false;
  }
    ...
    const observer = new MutationObserver(() => {
      ...
      dlBtn.addEventListener("click", async (e) => {
        ...
        try {
          ...
          setTimeout(() => { dlBtn.innerHTML = `${ICO.dl} Descargar`; }, 2500);
        } catch (err) {
          log.e(err.message);
          dlBtn.innerHTML = `${ICO.dl} Descargar`;
        }
  ...
  function watchStories() {
    ...
    const observer = new MutationObserver(() => {
      ...
      dlBtn.addEventListener("click", async (e) => {
        ...
        const vSrc = video?.src || video?.currentSrc || video?.querySelector("source")?.src;
        if (video && vSrc) {
