...
(function () {
  ...
  async function autoScrollScan(onMedia, onProgress, signal) {
    ...
    const seenMids = new Set(); // All message IDs we've ever seen
    const scannedMids = new Set(); // Messages we've already extracted media from
    let scrollAttempts = 0;
    const MAX_ATTEMPTS = 600;
    let staleRounds = 0; // Rounds without ANY new message IDs
    const MAX_STALE = 12;
    ...
    // Scan visible messages for media, serializing data immediately
    function scanCurrentView() {
      let newCount = 0;
      document.querySelectorAll(SEL[v].message).forEach((msg) => {
        const mid = msg.dataset?.mid || "";
        if (!mid) return; // Skip messages without ID — can't deduplicate
        if (scannedMids.has(mid)) return; // Already scanned this message
        scannedMids.add(mid);

        const items = findMediaInMessage(msg, v);
        // Count per-type to generate stable keys like mid_photo_0, mid_photo_1
        const typeCounts = {};
        items.forEach((item) => {
          const t = item.type;
    ...
    // Step 2: Scroll up repeatedly
    // Key insight: Telegram K loads history asynchronously via MTProto when
    // scrollTop approaches 0. We must actively POLL for new messages after
    // each scroll rather than using a fixed delay.
    while (scrollAttempts < MAX_ATTEMPTS) {
      if (signal?.aborted) break;

      // Scroll up
      scrollEl.scrollTop = Math.max(0, scrollEl.scrollTop - scrollEl.clientHeight * 0.8);
      await sleep(300); // Small initial wait

      // Active polling: wait up to 3s for new messages to appear
      let gotNew = false;
      for (let poll = 0; poll < 8; poll++) {
        ...
        if (newMidsFound > 0) {
          gotNew = true;
          break; // Got new messages, move on
        }
        // If scrollTop changed (Telegram adjusted it), keep waiting
        if (scrollEl.scrollTop > 10) break; // Not at top, no need to poll
      }

      // Scan for media in current view
      const newMediaFound = scanCurrentView();

      if (!gotNew && newMediaFound === 0) {
        staleRounds++;
        if (staleRounds >= MAX_STALE) {
          // Double check: try forcing scroll to 0 one more time
          ...
          if (lastNew === 0) {
            ...
            staleRounds = 0; // Got more, keep going
          }
        }
      } else {
        staleRounds = 0;
    ...
    // Step 3: Scroll back to bottom
    scrollEl.scrollTop = scrollEl.scrollHeight;

    // Clean up stale element references
    ...
    log.i(`Auto-scroll scan completo: ${allMedia.length} media en ${scrollAttempts} scrolls (${seenMids.size} msgs vistos)`);
    return allMedia;
  }
