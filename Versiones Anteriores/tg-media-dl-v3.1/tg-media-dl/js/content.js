...
(function () {
  ...
  async function autoScrollScan(onMedia, onProgress, signal) {
    ...
    while (scrollAttempts < MAX_ATTEMPTS) {
      ...
      const prevMidCount = seenMids.size;

      // Scroll up — use a big jump
      scrollEl.scrollTop = Math.max(0, scrollEl.scrollTop - scrollEl.clientHeight * 0.8);

      // Wait for Telegram to load new messages
      // If we hit scrollTop near 0, wait extra for async history loading
      if (scrollEl.scrollTop < 5) {
        await sleep(1200); // Telegram needs time to fetch from MTProto
      } else {
        await sleep(500);
      }
      ...
      if (newMidsFound === 0) {
        staleRounds++;
        // If at top AND truly stale, give extra chances
        if (scrollEl.scrollTop < 5 && staleRounds >= MAX_STALE) {
          log.i(`Top del chat alcanzado (${staleRounds} rondas sin mensajes nuevos)`);
          break;
        }
        if (staleRounds >= MAX_STALE * 2) {
          log.i(`Detenido: ${staleRounds} rondas sin mensajes nuevos`);
          break;
        }
      } else {
        staleRounds = 0;
