...
(function () {
  ...
  async function autoScrollScan(onMedia, onProgress, signal) {
    ...
    function scanCurrentView() {
      ...
      msgs.forEach((msg) => {
        findMediaInMessage(msg, v).forEach((item) => {
          ...
          // Serialize the item — clone important data because DOM elements
    ...
    // Track message IDs to detect new messages loaded (not scroll position!)
    // Telegram K adjusts scrollTop when loading history, so scroll position is unreliable
    ...
    let prevMids = getVisibleMids();
    let prevMediaCount = allMedia.length;
    ...
    while (scrollAttempts < MAX_ATTEMPTS) {
      ...
      // Scroll up
      scrollEl.scrollTop = Math.max(0, scrollEl.scrollTop - scrollEl.clientHeight * 0.7);
      await sleep(700);

      // Scan new content
      const newFound = scanCurrentView();

      // Check if we got new messages (by data-mid)
      const currentMids = getVisibleMids();
      let hasNewMids = false;
      for (const mid of currentMids) {
        if (!prevMids.has(mid)) { hasNewMids = true; break; }
      }

      // If we're at scrollTop 0 and no new messages appeared, we've reached the top
      const atTop = scrollEl.scrollTop <= 1;

      if (newFound === 0 && !hasNewMids && atTop) {
        noNewCount++;
        if (noNewCount >= 5) {
          log.i("Llegamos al inicio del chat");
          break;
        }
      } else if (newFound === 0 && !hasNewMids && !atTop) {
        // Not at top but no new content — maybe loading, wait more
        noNewCount++;
        await sleep(500); // Extra wait for lazy load
        if (noNewCount >= 20) {
          log.i("Sin media nuevo después de muchos intentos");
          break;
        }
      } else {
        noNewCount = 0;
      ...
      prevMids = currentMids;
      prevMediaCount = allMedia.length;
      scrollAttempts++;
      if (onProgress) onProgress("scanning", allMedia.length, scrollAttempts);

      // Log progress every 25 scrolls
      if (scrollAttempts % 25 === 0) {
        log.i(`Scan progreso: ${allMedia.length} media en ${scrollAttempts} scrolls (mids: ${currentMids.size})`);
      }
    }
