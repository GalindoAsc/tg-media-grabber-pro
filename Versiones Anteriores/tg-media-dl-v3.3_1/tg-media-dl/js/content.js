  ...
  async function sharedMediaScan(onMedia, onProgress, signal) {
    ...
    if (!searchSuper) {
      ...
      for (const sel of clickTargets) {
        ...
        if (el) { el.click(); await sleep(600); break; }
      }

      // Wait for sidebar & search-super to appear
      for (let i = 0; i < 25; i++) {
    ...
    if (!searchSuper) {
      log.w("No se encontró .search-super. Fallback a chat scroll...");
      return chatScrollScan(onMedia, onProgress, signal);
    }

    log.i("search-super encontrado, buscando tabs...");
    ...
    log.i(`Encontrados ${tabEls.length} tabs en nav`);
    ...
    log.i(`Tabs mapeados: ${Object.keys(tabMap).join(", ")}`);
    ...
    const scrollContainer = document.querySelector(
      "#column-right .sidebar-content > .scrollable.scrollable-y"
    ) || searchSuper.closest(".scrollable");

    if (!scrollContainer) {
      log.w("No scrollable container found");
      scanGridItems();
      // Still close and return whatever we got
    } else {
      // Scan Media tab
      ...
      for (const tabType of mediaTabTypes) {
        ...
        if (!tab) { log.i(`Tab "${tabType}" no encontrado, skip`); continue; }

        // Skip hidden tabs (class contains "hide")
        if (tab.classList.contains("hide")) { log.i(`Tab "${tabType}" oculto, skip`); continue; }

        // Click the tab
        log.i(`Clickeando tab: ${tabType}...`);
        tab.click();
        await sleep(800);
        ...
        while (scrollAttempts < 300 && staleRounds < MAX_STALE) {
          if (signal?.aborted) break;

          const prevCount = allMedia.length;

          // Scroll down in the sidebar
          scrollContainer.scrollTop += scrollContainer.clientHeight * 0.7;
          await sleep(350);

          // Poll for new items appearing
          for (let poll = 0; poll < 4; poll++) {
            await sleep(300);
            const newItems = scanFn();
            if (newItems > 0) break;
          }
        ...
        log.i(`Tab "${tabType}" completo: ${allMedia.length} items total`);
      }
    }

    // Step 4: Close sidebar
    await sleep(300);
    const closeBtn = document.querySelector("#column-right .sidebar-close-button");
